...
import { listSavedRuns, listSavedComparisons } from "@/lib/api";
import type { SavedRunSummary, SavedComparisonSummary } from "@/lib/types";
...
export default function CatalogPage() {
  ...
  const [savedComps, setSavedComps] = useState<SavedComparisonSummary[]>([]);
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    Promise.all([listSavedRuns(), listSavedComparisons()])
      .then(([runs, comps]) => {
        if (active) {
          setSavedRuns(runs);
          setSavedComps(comps);
  ...
  return (
    <div className="flex h-screen bg-neutral-950 text-neutral-100 flex-col overflow-auto">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full p-8">
        ...
        {loading ? (
          <div className="text-neutral-400">Loading database records...</div>
        ) : activeTab === "reports" ? (
          savedRuns.length === 0 ? (
            <div className="text-neutral-400 p-8 text-center border border-dashed border-white/10 rounded-xl">No saved target reports found.</div>
          ) : (
          ...
          savedComps.length === 0 ? (
            <div className="text-neutral-400 p-8 text-center border border-dashed border-white/10 rounded-xl">No saved comparisons found.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {savedComps.map((comp) => (
                <Link key={comp.id} href={`/compare/${comp.id}`}>
                  <div className="group border border-white/10 bg-white/5 rounded-2xl p-5 hover:bg-white/10 hover:border-pink-500/50 transition cursor-pointer flex flex-col h-[140px]">
                    <div className="flex-1">
                      ...
                      <div className="text-xs font-medium text-pink-400 mt-2 uppercase tracking-wide">
                        Comparison
                      </div>
                    </div>
                    <div className="flex items-center justify-between mt-4">
